...
const MedicalRecords = () => {
  const medicalHistory = useSelector(
    (state: RootState) => state.auth.user?.medicalHistory
  );

  return (
    <div className="w-full h-full p-6 shadow-md bg-transparent flex flex-col">
      <div>
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="mt-2 space-y-1">
                <p>
                  <strong>Name:</strong>{" "}
                  {medicalHistory?.emergencyContact?.name || "N/A"}
                </p>
                <p>
                  <strong>Relationship:</strong>{" "}
                  {medicalHistory?.emergencyContact?.relationship || "N/A"}
                </p>
                <p>
                  <strong>Phone Number:</strong>{" "}
                  {medicalHistory?.emergencyContact?.phoneNumber || "N/A"}
                </p>
              </div>
            </div>
            <Separator />
              ...
              {medicalHistory?.medicalConditions?.length > 0 ? (
                medicalHistory.medicalConditions.map((condition, index) => (
                  <div key={index} className="mt-2 space-y-1">
                    <p>
                      <strong>Condition:</strong> {condition?.name || "Unknown"}
                    </p>
                    <p>
                      <strong>Diagnosis Date:</strong>{" "}
                      {condition?.diagnosisDate || "Unknown"}
                    </p>
                    <p>
              ...
              {medicalHistory?.medications?.length > 0 ? (
                medicalHistory.medications.map((medication, index) => (
                  <div key={index} className="mt-2 space-y-1">
                    ...
                    <p>
                      <strong>Dosage:</strong> {medication?.dosage || "Unknown"}
                    </p>
                    <p>
              ...
              {medicalHistory?.surgeries?.length > 0 ? (
                medicalHistory.surgeries.map((surgery, index) => (
                  <div key={index} className="mt-2 space-y-1">
                    ...
                    <p>
                      <strong>Date:</strong> {surgery?.date || "Unknown"}
                    </p>
                    <p>
                      <strong>Outcome:</strong> {surgery?.outcome || "Unknown"}
                    </p>
                  </div>
                ))
              ) : (
                ...
                medicalHistory.immunizations.map((immunization, index) => (
                  <div key={index} className="mt-2 space-y-1">
                    <p>
                      ...
                      {immunization?.vaccine || "Unknown"}
                    </p>
                    <p>
                      <strong>Date:</strong> {immunization?.date || "Unknown"}
                    </p>
                    <p>
                      <strong>Booster Date:</strong>{" "}
                      {immunization?.boosterDate || "N/A"}
                    </p>
                  </div>
                ))
              ) : (
              ...
              {medicalHistory?.familyHistory?.length > 0 ? (
                medicalHistory.familyHistory.map((history, index) => (
                  <div key={index} className="mt-2 space-y-1">
                    <p>
                      <strong>Relationship:</strong>{" "}
                      {history?.relationship || "Unknown"}
                    </p>
                    <p>
                      <strong>Conditions:</strong>{" "}
                      {history?.conditions?.join(", ") || "None"}
                    </p>
                    <p>
                      <strong>Age of Onset:</strong>{" "}
                      {history?.ageOnset || "Unknown"}
                    </p>
                  </div>
                ))
              ) : (
