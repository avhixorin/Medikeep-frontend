...
import { AiChatThread } from "@/components/Dashboard/Records/types";
import { AIChatMessage } from "@/components/Dashboard/Records/types";
import { v4 as uuid } from "uuid";
import { useFetchAiChatThreads } from "@/components/Dashboard/hooks/dataHooks";
...
export const CreativeChatInterface = ({ entityId }: { entityId: string }) => {
  ...
  const [activeThread, setActiveThread] = useState<AiChatThread>(
    createDefaultThread()
  );
  ...
  useEffect(() => {
    if (data) {
      if (
        ...
        activeThread.chatThread.length === 0
      ) {
  ...
  const handleSelectThread = (threadId: string) => {
    const threadToSelect = data?.find((t) => t._id === threadId);
    if (threadToSelect) {
  ...
  const handleSendMessage = (message: string) => {
    ...
    setActiveThread((current) => ({
      ...current!,
      chatThread: [...current!.chatThread, userMsg],
    }));
    setIsSending(true);
  ...
  const simulateAIResponse = (userMessage: string) => {
    setTimeout(() => {
      ...
      setActiveThread((current) => ({
        ...current!,
        chatThread: [...current!.chatThread, aiResponse],
      }));
      setIsSending(false);
    }, 1500);
  };
