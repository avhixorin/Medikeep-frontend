...
export default function Welcome() {
  ...
  const handleNavigation = (title: string) => {
    if (title === "Upcoming Appointments") {
      ...
      navigate.push("/dashboard/medical-records");
    } else if (title === "Messages") {
      navigate.push("/dashboard/chats");
    } else {
      // if (user?.role === "doctor") {
      //   navigate.push("/dashboard/patients");
      // } else {
      navigate.push("/dashboard/vitals");
      // }
    }
  };
          ...
          {cards.map((card, index) => (
            ...
            >
              <div className="flex flex-col h-full space-y-2">
                ...
                <CardFooter>
                  <Button
                    className="w-full py-2 bg-foreground text-background"
